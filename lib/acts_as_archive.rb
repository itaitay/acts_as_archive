      ...
      def acts_as_archive(options={})
        ...
        if options[:archive]
          ...
          if options[:auto_archive]
            archive_job_class = "#{options[:class]}::AutoArchiveJob"
            $stdout.puts "Defining Job #{archive_job_class} with auto_archive scope"

            scope :auto_archive, options[:auto_archive][:condition]

            if defined?(Sidekiq)
                eval <<-EVAL
                class #{archive_job_class}
                  include Sidekiq::Worker

                  def perform
                    $stdout.puts "Starting auto archiving of #{self}"
                    amount_of_records = #{self}.auto_archive.size
                    $stdout.puts "Found \#{amount_of_records} records to archive"
                    #{self}.auto_archive.delete_all
                    #{archive_job_class}.perform_in(#{options[:auto_archive][:interval] || 1.day})
                  end
                end
              EVAL
              auto_archive_klass = eval("::#{archive_job_class}")
              auto_archive_klass.perform_in(options[:auto_archive][:interval] || 1.day)
            end
